...
k.nearrest.neighbors <- function(i, distances, k = 25) {
...
most.probable.packages <- function(user, user.package.matrix, distance, k = 25) {
    return(order(sapply(1:ncol(user.package.matrix),
                        function(package) {
                            installation.probability(user, package, user.package.matrix, distance, k = k)
                        }), decreasing = T))
}
